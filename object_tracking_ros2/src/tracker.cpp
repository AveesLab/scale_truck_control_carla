...
// cv::Rect ---> box bbox cx cy s r
float Tracker::CalculateIoU(const box& bbox, const Track& track) {
    auto trk = track.GetStateAsBbox();
    // cout << bbox.x << ", " << trk.x << endl;
    float bbox_x1 = bbox.x - bbox.w / 2;
    float bbox_x2 = bbox.x + bbox.w / 2;
    float bbox_y1 = bbox.y - bbox.h / 2;
    float bbox_y2 = bbox.y + bbox.h / 2;
